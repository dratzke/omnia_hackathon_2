...
impl Plugin for WorldPlugin {
    fn build(&self, app: &mut App) {
        ...
        if self.physics {
            ...
            app.add_systems(Update, apply_gravity_modification);
        }
    }
}
...
fn process_potential_collision(
    ...
    if let Ok((mut last_touched_id, mut last_touch_time, mut gravity_modifier)) =
        players.get_mut(potential_player)
    {
...
fn process_potential_collision_stop(
    ...
    // Only proceed if the entities match our requirements
    if let Ok((_, mut last_touch_time, mut gravity_modifier)) = players.get_mut(potential_player) {
